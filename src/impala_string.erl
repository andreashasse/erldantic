-module(impala_string).

-export([from_string/3, to_string/3]).

-ignore_xref([{impala_string, from_string, 3}, {impala_string, to_string, 3}]).
...
-spec from_string(TypeInfo :: impala:type_info(),
...
from_string(_TypeInfo, #im_simple_type{type = NotSupported} = T, _String)
...
from_string(_TypeInfo, #im_simple_type{type = PrimaryType}, String) ->
...
from_string(_TypeInfo, #im_remote_type{mfargs = {Module, TypeName, Args}}, String) ->
    TypeInfo = impala_module_types:get(Module),
    TypeArity = length(Args),
    {ok, Type} = impala_type_info:get_type(TypeInfo, TypeName, TypeArity),
    TypeWithoutVars = apply_args(TypeInfo, Type, Args),
    from_string(TypeInfo, TypeWithoutVars, String);
from_string(_TypeInfo, #im_literal{value = Literal}, String) ->
    try_convert_string_to_literal(Literal, String);
from_string(TypeInfo, #im_union{} = Type, String) ->
...
-spec to_string(TypeInfo :: impala:type_info(),
                Type :: impala:im_type_or_ref(),
                Data :: term()) ->
                   {ok, string()} | {error, [impala:error()]}.
...
to_string(_TypeInfo, #im_simple_type{type = NotSupported} = T, _Data)
    when NotSupported =:= pid
         orelse NotSupported =:= port
         orelse NotSupported =:= reference
         orelse NotSupported =:= bitstring
         ...
         orelse NotSupported =:= none ->
    erlang:error({type_not_supported, T});
to_string(_TypeInfo, #im_simple_type{type = PrimaryType}, Data) ->
...
to_string(_TypeInfo, #im_remote_type{mfargs = {Module, TypeName, Args}}, Data) ->
    TypeInfo = impala_module_types:get(Module),
    TypeArity = length(Args),
    {ok, Type} = impala_type_info:get_type(TypeInfo, TypeName, TypeArity),
    TypeWithoutVars = apply_args(TypeInfo, Type, Args),
    to_string(TypeInfo, TypeWithoutVars, Data);
to_string(_TypeInfo, #im_literal{value = Literal}, Data) ->
    try_convert_literal_to_string(Literal, Data);
to_string(TypeInfo, #im_union{} = Type, Data) ->
    ...
    try
        ...
        error:badarg ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = integer}, value => String}}]}
    end;
    ...
    try
        ...
        error:badarg ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = float}, value => String}}]}
    end;
...
do_convert_string_to_type(boolean, String) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = boolean}, value => String}}]};
    ...
    try
        ...
        error:badarg ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = atom}, value => String}}]}
    end;
    ...
    case convert_string_to_type(atom, String) of
        ...
        {ok, _Other} ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_literal{value = Literal}, value => String}}]};
        {error, Reason} ->
            {error, Reason}
    end;
...
union(Fun, TypeInfo, #im_union{types = Types} = T, String) ->
...
lits_to_charlist(Data) ->
    case application:get_env(impala, check_unicode, false) of
        true ->
...
union_to_string(TypeInfo, #im_union{types = Types} = T, Data) ->
