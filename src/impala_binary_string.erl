-module(impala_binary_string).

-export([from_binary_string/3, to_binary_string/3]).

-ignore_xref([{impala_binary_string, from_binary_string, 3},
              {impala_binary_string, to_binary_string, 3}]).
...
-doc(#{params =>
           #{"BinaryString" => "The binary string value to convert to Erlang format",
             "Type" => "The type specification (impala:im_type_or_ref())",
             "TypeInfo" => "The type information containing type definitions"}}).

-spec from_binary_string(TypeInfo :: impala:type_info(),
...
from_binary_string(_TypeInfo, #im_simple_type{type = NotSupported} = T, _BinaryString)
...
from_binary_string(_TypeInfo, #im_simple_type{type = PrimaryType}, BinaryString) ->
...
from_binary_string(_TypeInfo, #im_literal{value = Literal}, BinaryString) ->
    try_convert_binary_string_to_literal(Literal, BinaryString);
from_binary_string(TypeInfo, #im_union{} = Type, BinaryString) ->
...
-spec to_binary_string(TypeInfo :: impala:type_info(),
                       Type :: impala:im_type_or_ref(),
                       Data :: term()) ->
                          {ok, binary()} | {error, [impala:error()]}.
...
to_binary_string(_TypeInfo, #im_simple_type{type = NotSupported} = T, _Data)
    when NotSupported =:= pid
         orelse NotSupported =:= port
         orelse NotSupported =:= reference
         orelse NotSupported =:= bitstring
         ...
         orelse NotSupported =:= none ->
    erlang:error({type_not_supported, T});
to_binary_string(_TypeInfo, #im_simple_type{type = PrimaryType}, Data) ->
...
to_binary_string(_TypeInfo, #im_remote_type{mfargs = {Module, TypeName, Args}}, Data) ->
    TypeInfo = impala_module_types:get(Module),
    TypeArity = length(Args),
    {ok, Type} = impala_type_info:get_type(TypeInfo, TypeName, TypeArity),
    TypeWithoutVars = apply_args(TypeInfo, Type, Args),
    to_binary_string(TypeInfo, TypeWithoutVars, Data);
to_binary_string(_TypeInfo, #im_literal{value = Literal}, Data) ->
    try_convert_literal_to_binary_string(Literal, Data);
to_binary_string(TypeInfo, #im_union{} = Type, Data) ->
    ...
    try
        ...
        error:badarg ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = integer}, value => BinaryString}}]}
    end;
    ...
    try
        ...
        error:badarg ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = float}, value => BinaryString}}]}
    end;
...
do_convert_binary_string_to_type(boolean, BinaryString) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = boolean}, value => BinaryString}}]};
    ...
    try
        ...
        error:badarg ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = atom}, value => BinaryString}}]}
    end;
...
do_convert_binary_string_to_type(nonempty_string, <<>>) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = nonempty_string}, value => <<>>}}]};
...
do_convert_binary_string_to_type(nonempty_binary, <<>>) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = nonempty_binary}, value => <<>>}}]};
    ...
    case do_convert_binary_string_to_type(integer, BinaryString) of
        ...
        {ok, Value} ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = non_neg_integer}, value => Value}}]};
        {error, Reason} ->
            {error, Reason}
    end;
    ...
    case do_convert_binary_string_to_type(integer, BinaryString) of
        ...
        {ok, Value} ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = pos_integer}, value => Value}}]};
        {error, Reason} ->
            {error, Reason}
    end;
    ...
    case do_convert_binary_string_to_type(integer, BinaryString) of
        ...
        {ok, Value} ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = neg_integer}, value => Value}}]};
        {error, Reason} ->
            {error, Reason}
    end;
do_convert_binary_string_to_type(Type, BinaryString) ->
    ...
    case convert_binary_string_to_type(atom, BinaryString) of
        ...
        {ok, _Other} ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_literal{value = Literal}, value => BinaryString}}]};
        {error, Reason} ->
            {error, Reason}
    end;
...
union(Fun, TypeInfo, #im_union{types = Types} = T, BinaryString) ->
...
convert_type_to_binary_string(integer, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = integer}, value => Data}}]};
...
convert_type_to_binary_string(float, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = float}, value => Data}}]};
...
convert_type_to_binary_string(number, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = number}, value => Data}}]};
...
convert_type_to_binary_string(boolean, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = boolean}, value => Data}}]};
...
convert_type_to_binary_string(atom, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = atom}, value => Data}}]};
    ...
    case unicode:characters_to_binary(Data) of
        ...
        _Other ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = string}, value => Data}}]}
    end;
    ...
    case unicode:characters_to_binary(Data) of
        ...
        _Other ->
            {error,
             [#im_error{type = type_mismatch,
                        location = [],
                        ctx = #{type => #im_simple_type{type = nonempty_string}, value => Data}}]}
    end;
...
convert_type_to_binary_string(binary, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = binary}, value => Data}}]};
...
convert_type_to_binary_string(nonempty_binary, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = nonempty_binary}, value => Data}}]};
...
convert_type_to_binary_string(non_neg_integer, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = non_neg_integer}, value => Data}}]};
...
convert_type_to_binary_string(pos_integer, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = pos_integer}, value => Data}}]};
...
convert_type_to_binary_string(neg_integer, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_simple_type{type = neg_integer}, value => Data}}]};
...
try_convert_literal_to_binary_string(Literal, Data) ->
    {error,
     [#im_error{type = type_mismatch,
                location = [],
                ctx = #{type => #im_literal{value = Literal}, value => Data}}]}.

union_to_binary_string(TypeInfo, #im_union{types = Types} = T, Data) ->
