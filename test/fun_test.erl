...
erl_abstract_code_parses_fun_types_test() ->
    ...
    ?assertEqual({type, 'fun'}, maps:get({type, fun1, 0}, TypeInfo)),
    ...
    ?assertEqual(#a_function{args = [{type, integer}, {type, atom}], return = {type, 'fun'}},
    ...
    ?assertEqual(#a_rec{name = with_fun,
                        fields = [{id, {type, integer}}, {handler, {type, 'fun'}}]},
