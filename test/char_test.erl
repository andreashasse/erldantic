...
-spec to_json(base64()) -> {ok, json:json()} | {error, [spectra:error()]}.
to_json(Value) ->
    spectra:encode(json, ?MODULE, base64, Value).

-spec from_json(binary()) -> {ok, base64()} | {error, [spectra:error()]}.
from_json(Json) ->
    spectra:decode(json, ?MODULE, base64, Json).

fancy_string_test() ->
    %% This test documents that spectra currently doesn't make a difference between char and int.
    OkVal = "hello",
    BadVal = "Hello",
...
-spec to_fancy_string_json(my_fancy_string()) ->
    {ok, json:json()} | {error, [spectra:error()]}.
to_fancy_string_json(Value) ->
    case spectra:encode(json, ?MODULE, my_fancy_string, Value) of
        {ok, Json} ->
...
-spec from_fancy_json(binary()) -> {ok, my_fancy_string()} | {error, [spectra:error()]}.
from_fancy_json(Json) ->
    spectra:decode(json, ?MODULE, my_fancy_string, Json).
