...
%% - Complex JSON decoding with type validation
%%
%% Usage:
%%   From Erlang shell with test profile:
...
run() ->
    ...
    io:format("File size: ~p bytes (~.2f KB)~n", [Size, Size / 1024]),

    io:format("~nRunning decode benchmark...~n"),
    ...
    Times = [
        begin
            {Time, _Result} = timer:tc(DecodeFun),
            Time
        end
    ...
    io:format("~nThroughput:  ~.2f decodes/sec~n", [1000000 / MeanTime]),
