...
person_module_test_() ->
    {setup,
     ...
     fun noop/1,
     fun(_SetupData) -> {inparallel, [person_type_is_record(), person_person(), person_address(), name_t()]} end}.
...
person_type_is_record() ->
    ...
    [?_assertEqual({address, <<"mojs">>, <<"sollentuna">>}, person:address_from_json(Json))].
...
person_person() ->
    Json =
        json:decode(<<"{\"name\": {\"first\": \"Andreas\", \"last\": \"Hasselberg\"}, "
                      ...
                      "}}"/utf8>>),
    {person, Name, Age, Home} = person:person_from_json(Json),
...
name_t() ->
    Json =
        json:decode(<<"{\"first\": \"Andreas\", \"last\": \"Hasselberg\", \"not_present\": "
                      "22}"/utf8>>),
    M = person:name_t_from_json(Json),
    Expect = #{first => <<"Andreas">>, last => <<"Hasselberg">>},
    [?_assertEqual(Expect, M),
    ?_assertEqual([first, last], maps:keys(M))].
