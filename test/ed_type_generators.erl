...
-export([ed_type/0, ed_simple_type/0, ed_tuple/0, ed_map/0, ed_rec/0,
         ed_type_with_variables/0, ed_function/0, ed_union/0, ed_literal/0,
         ed_rec_ref/0, ed_remote_type/0, ed_maybe_improper_list/0,
         ed_nonempty_improper_list/0, ed_user_type_ref/0, ed_var/0,
         ed_range/0, ed_list/0, ed_nonempty_list/0, map_field/0]).

%% Simple type generator
simple_type_atom() ->
    oneof([string, nonempty_string, integer, non_neg_integer, neg_integer,
           pos_integer, float, number, boolean, binary, nonempty_binary,
           bitstring, nonempty_bitstring, atom, term, reference, pid, port,
           iolist, iodata, none, map]).

ed_simple_type() ->
    ?LET(Type, simple_type_atom(),
         #ed_simple_type{type = Type}).

%% Tuple generator
ed_tuple() ->
    oneof([
        #ed_tuple{fields = any},
        ?LET(Fields, list(ed_type()),
             #ed_tuple{fields = Fields})
    ]).

%% Map field generator  
map_field() ->
    oneof([
        ?LET({Name, Type}, {atom(), ed_type()},
             {map_field_assoc, Name, Type}),
        ?LET({Name, Type}, {atom(), ed_type()},
             {map_field_exact, Name, Type}),
        ?LET({KeyType, ValueType}, {ed_type(), ed_type()},
...
ed_rec() ->
    ?LET({Name, Fields, Arity},
         ...
         #ed_rec{name = Name, fields = Fields, arity = Arity}).

%% Type with variables generator
ed_type_with_variables() ->
    ?LET({Type, Vars}, {ed_type(), list(atom())},
...
ed_function() ->
    oneof([
        ...
        ?LET({Args, Return}, {list(ed_type()), ed_type()},
             #ed_function{args = Args, return = Return})
    ]).

%% Union generator
ed_union() ->
    ?LET(Types, non_empty(list(ed_type())),
...
record_field() ->
    ?LET({FieldName, Type}, {atom(), ed_type()},
         {FieldName, Type}).
...
ed_maybe_improper_list() ->
    ?LET({Elements, Tail}, {ed_type(), ed_type()},
...
ed_user_type_ref() ->
    ?LET({TypeName, Variables}, {atom(), list(ed_type())},
...
%% Range generator
ed_range() ->
    ?LET({Lower, Upper}, {integer(), integer()},
